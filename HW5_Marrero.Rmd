...
2. Them we will create 20 resampled datasets and build trees predicting `log(LPRICE)` with them. 
...
3. Then, we will make predictions on the holdout sample for each of the 20 tree models.
...
4. Then we average those predictions to make a 'bagged model' prediction.  
...
5. We will then find the errors for each prediction of the model (not the bagged-model prediction).
```{r}
...
6. Finally, we will find the MSE for each of the tree models, as well as the bagged model. 
```{r}
...
The bagged model outperforms 100% of the tree models in our code. In particular, the MSE of the bagged model was in fact lower in value than every single MSE of the tree models.   
...
Using the same training data as above, we will create and test forest models as following:   

1. First, we will make a 20-tree forest predicting `log(LPRICE)` using `ranger`.  
...
3. Then, we will make predictions for both forests on the holdout sample.  
...
4. Finally, we will find the the *out-of-sample* MSE for each each forest.
...
When comparing the forests to the bagged model, the forests were bigger in terms of having a larger MSE value. Diving deeper, the forests focused on subsets rather than examining single variables. 
