...
### Lamdas 
From this model, we will now find the $\lambda$ that minimizes OOS deviance (hint: see `cv.glmnet` help page) and the deviance at that $\lambda$ (no need to report either). What is the number of non-zero coefficients with that $\lambda_{min}$? 
```{r}
...
Which model has the smaller deviance? In at most two sentences, why can't we use deviances to compare Model 6 and Model 7? 

Rerun model 7 but set `type.measure="mse"` in `cv.glmnet`. 
```{r}
mod7 <- cv.glmnet(xmatrix_22_log, homes$twotwo,type.measure="mse")
```
In at most two sentences, Can we compare these models now? and if so, which is better? 
